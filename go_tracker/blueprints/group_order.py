...
from go_tracker.models import GroupOrder, Provider
from go_tracker.models.group_order import OrderStatus

group_order = Blueprint("group_order", __name__, url_prefix="/api/order")


@group_order.route("", methods=["GET", "POST", "PATCH", "DELETE"], defaults={"pk": ""})
@group_order.route("<string:pk>", methods=["GET", "POST", "PATCH", "DELETE"])
    ...
    match request.method:
        case "GET":
            if pk:
                ...
                if show_completed:
                    ...
                    orders = [
                        order
                        for order in GroupOrder.find(
                            GroupOrder.status != OrderStatus.DELIVERED
                        ).all()
                    ]
        ...
        case "POST":
            ...
            try:
                if req.get("provider") is None:
                    return Response(
                        {"error": "provider is required"}, status=HTTPStatus.BAD_REQUEST
                    )
                else:
        ...
        case "PATCH":
            ...
            try:
                ...
                order.update(**req)
                return Response(
                    order.json(), status=HTTPStatus.OK, content_type="application/json"
                )
            except ValidationError as e:
                return Response(
                    {"error": str(e)},
